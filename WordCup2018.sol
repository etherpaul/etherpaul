...
/**
 * @title UEFATeamToken
 * @dev The team token. One token represents a team. UEFATeamToken is also an ERC20 standard token.
 */
contract UEFATeamToken is StandardToken, Ownable {
    ...
    */
    ...
    /**
    * @dev Game opponent, gameOpponent is also a UEFATeamToken.
    */
    ...
    *
    * Override ERC20 transfer token function. If the _to address is not this UEFATeamToken,
    * then call the super transfer function, which will be ERC20 token transfer.
    * Otherwise, the user want to sell the token (UEFATeamToken -> ETH).
    ...
    *
    * User send ETH to this UEFATeamToken, then his token balance will be increased based on price.
    ...
    */
    function endGame(address _gameOpponent, uint8 _gameResult) onlyOwner public {
        ...
        UEFATeamToken op = UEFATeamToken(gameOpponent);
        if (_gameResult == 1) {
            // Lose
